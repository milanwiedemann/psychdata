...
calc_sum <- function(data, id_str, var_str, session_str, n_min){
  ...
  for (i in 1:length(session_str)) {
    ...
    var_str_i <- paste(var_str, "sum", substr(x = session_str[i], start = 1, stop = (nchar(session_str[i]) - 1)), sep = "_")
    ...
    # Calculate mean
    data_select_var_ses_mean <- data_select_var_ses %>%
      mutate(!!var_str_i := calc_mean_n_min(.[, 2:(item_count + 1)] * item_count, n_min))
